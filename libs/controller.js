...
PDB.controller = {
  ...
  refreshGeometryByMode: function (mode) {
    console.log("[PDB.controller] refreshGeometryByMode called with mode:", mode);
    var pdbId = PDB.pdbId;
    if (!pdbId) {
      console.warn("refreshGeometryByMode: No PDB ID loaded");
      return;
    }

    // 1. Clear existing Main Structure
    PDB.render.clear(0); // Main
    PDB.render.clear(2); // Structure groups

    // 2. Set Config
    PDB.config.mainMode = mode;

    // 3. Request w3m to update its internal geometry data
    // w3m.representation maps to w3m.api.switchRepModeMain(mode)
    if (typeof w3m !== 'undefined' && w3m.representation) {
      w3m.representation(w3m.ATOM_MAIN, mode);
    }

    // 4. Rebuild Three.js Groups from updated w3m data
    if (PDB.loader.renderGroups) {
      PDB.loader.renderGroups(pdbId);
    } else {
      console.error("PDB.loader.renderGroups missing! Update loader.js");
    }

    console.log("[PDB.controller] Geometry refreshed.");
  },
...
document.getElementById("toggleNavigation").addEventListener("click", function () {
  ...
  if (PDB.mode === PDB.MODE_THREE) {
    ...
    alert("WASD navigation is only available in non-VR mode");
  }
});
...
onKeyDown: function (event) {
  ...
  if (e.keyCode === 13) { // Enter key
    ...
    scope.requestRemote(input.value);
  } else if (e.keyCode === 78) { // 'N' key for Navigation toggle
    // Only enable in non-VR mode
    if (PDB.mode === PDB.MODE_THREE) {
      PDB.navigation.toggle();
    }
